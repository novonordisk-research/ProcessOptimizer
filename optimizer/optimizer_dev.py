...
class Optimizer(object):
    ...
    * `space`
        ...
        if (fit and self._n_initial_points <= 0 and
            ...
            if self.n_objectives > 1:
                ...
                for i in range(self.n_objectives):
                    est = clone(self.base_estimator_)
                    est.fit(self.space.transform(self.Xi), self.yi[i])
    ...
    def _check_y_is_valid(self, x, y):
        ...
        elif is_listlike(y) and is_listlike(x):
    ...
    def __ObjectiveGP(self, x):
        ...
        for i in range(self.NObj):
            F[i] = -self.models[i].predict(xx)[0] + Fator * Constraints
    
        return F


    def best_Pareto_point(self, pop, front):
