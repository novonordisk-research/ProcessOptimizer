...
def check_dimension(dimension, transform=None) -> Dimension:
    ...
    * `dimension`:
        ...
        - a list of tasks with embedded set to characterise active task: [task1, task2, ..., (active_task1)]
...
class Integer(Dimension):
    ...
    def _sample(self, point_list: Iterable[float]) -> np.ndarray:
        point_list = [
            min(point * (self.high + 1 - self.low) + self.low,self.high) for point in point_list
        ]
