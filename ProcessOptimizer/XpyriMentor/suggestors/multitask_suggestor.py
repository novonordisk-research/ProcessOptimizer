...
class MTSuggestor:
    def __init__(
        ...
        if self.kwargs.get("surrogate_kwargs").get("task_feature") is None:
            ...
            # Finds task feature
                ...
                self.kwargs["surrogate_kwargs"]["task_feature"] = (
                    self.space.task_feature
                )
            ...
            # If multiple task features raise error
                ...
                raise NotImplementedError(
                    "More than one task feature, e.g., '[0, 1, {1}]', provided in self.space. Please combine tasks into a single list as MTSuggestor does not currently support multiple task features."
                )
        ...
        if optimization_type == "multitask":
            ...
            if self.kwargs.get("surrogate_kwargs").get("covar_module") is None:
                ...
                non_task_bound_widths = [
                    self.space.bounds[i][1] - self.space.bounds[i][0]
                ...
                if (
                    self.kwargs.get("surrogate_kwargs").get(
                        "covar_relative_lengthscale"
                    )
                    is None
                ):
                    self.kwargs["surrogate_kwargs"]["covar_relative_lengthscale"] = 0.2

                rel_lengthscale = self.kwargs.get("surrogate_kwargs").get(
                    "covar_relative_lengthscale"
                )

                base_kernel.lengthscale = torch.tensor(
                    [[width * rel_lengthscale for width in non_task_bound_widths]]
                )

                self.kwargs["surrogate_kwargs"]["covar_module"] = ScaleKernel(
                    base_kernel
                )
    ...
    def acquisition_function(self, model: GP, sampler: SobolQMCNormalSampler) -> Any:
        acq_constraints = self.kwargs.get("acq_func_kwargs").get(
        ...
        if acq == "qLogExpectedImprovement" or acq is None:
            ...
            raise NotImplementedError(
                f"The acquisition function '{acq}' is not currently supported by MTSuggestor."
            )
    ...
    def suggest(
        ...
        self.active_task_indices = np.where(np.stack(Xi)[:, task_feature] == test_task)[
            0
        ]
        ...
        if not len(self.active_task_indices) == 0:
            ...
            raise ValueError(
                "Suggestion cannot be made as no data from the active task has been provided."
            )
        ...
        candidates = self.optimize_acqf_and_get_observation(
            acq_func, n_points_to_suggest
        ).numpy()

        return candidates

    def initialize_model(self, Xi: Iterable[Iterable], Yi: Iterable):
        # define likelihood
        ...
        if self.tasks == 1:
            ...
            if self.kwargs.get("surrogate_kwargs").get("type") == "multioutput":
                if not isinstance(likelihood, MultitaskGaussianLikelihood):
                    raise ValueError(
                        "Only MultitaskGaussianLikelihood is supported with this type of model. Use 'likelihood: None' in suggestor factory"
                    )
                ...
                model = MultiTaskGP(
                    train_X=train_x,
                    train_Y=train_y,
                    task_feature=self.kwargs.get("surrogate_kwargs").get(
                        "task_feature"
                    ),
                    likelihood=likelihood,
                    mean_module=mean_module,
    ...
    def optimize_acqf_and_get_observation(
        self, acq_func, n_points_to_suggest: int = 1, num_restarts: int = 10
    ):
    ...
    def preprocess_tell(self, x: Any, y: Any, task: Any):
        ...
        if ncols_x == self.space.n_dims:
            ...
            if not all_tasks_valid:
                raise ValueError(
                    f"Task feature entries in column {task_feature} of x are not valid tasks, i.e., not in {all_tasks_in_space}."
                )
            ...
            if not task is None:
                ...
                if not all_tasks_equal:
                    if is_x_tasks_unique:
                        warnings.warn(
                            f"Task feature value is different in column {task_feature} of x and 'task' argument. Defaulting to value in x."
                        )
                    else:
                        warnings.warn(
                            f"Task features in column {task_feature} of x have different values between entries. Ignoring 'task' argument"
                        )
            ...
            if task is None:
                ...
                if not np.isin(task, all_tasks_in_space):
                    raise ValueError(
                        f"task argument {task} not a valid task in space, i.e., not in {all_tasks_in_space}."
                    )
    ...
    def plot_objective(
        ...
        **kwargs,
        ...
        if Xi is None:
            warnings.warn(
                "No data told to the XpyriMentor, plotting the prior if this is supported."
            )
        ...
        X_linspaces = np.array(
            [
                np.linspace(lower_bounds[i], upper_bounds[i], grid_resolution, axis=0)
                for i in non_task_indices
            ]
        )
        ...
        X_grid_with_task = np.insert(
            X_grid,
            task_feature,
            np.ones((grid_resolution ** len(non_task_indices),)) * test_task,
            axis=1,
        )
            ...
            for j in non_task_indices:
                ...
                if i == j:
                    ...
                    with torch.no_grad():
                        ...
                        std_ij = (
                            np.sqrt(posterior.variance.numpy().squeeze())
                            if hasattr(posterior, "variance")
                            else np.zeros_like(mean_ij)
                        )
                    ...
                    cs = ax.contourf(
                        ...
                        levels=levels,
                        cmap="viridis_r",
                    )  # note swapped axes for correct orientation
                    fig.colorbar(
                        cs,
                        ax=axes[0][-1],
                        orientation="horizontal",
                        label=f"Expected value plot ({i}, {j})",
                    )
                    ax.set_xlabel(f"x_{j}")
                    ax.set_ylabel(f"x_{i}")

                    test_samples_i = [
                        sample[i] for sample in Xi if sample[task_feature] == test_task
                    ]
                    test_samples_j = [
                        sample[j] for sample in Xi if sample[task_feature] == test_task
                    ]
        ...
        legend_test_points = mpl.lines.Line2D(
            [], [], color="darkorange", marker=".", markersize=9, lw=0.0
        )
        # Training data
        legend_training_points = mpl.lines.Line2D(
            [], [], color="lightgrey", marker=".", markersize=9, lw=0.0
        )
        # Expected minimum x_star in 2D plots
        legend_hp = mpl.lines.Line2D(
            [], [], color="k", marker="D", markersize=5, lw=0.0
        )
...
class MTMeanGPModel(ExactGP):
    ...
    def __init__(self, train_x, train_y, likelihood, num_tasks_):
        ...
        self.mean_module = MultitaskMean(ConstantMean(), num_tasks=num_tasks_)
        self.covar_module = MultitaskKernel(RBFKernel(), num_tasks=num_tasks_, rank=1)
...
class MTGPModel(ExactGP):
    ...
    def __init__(self, train_x, train_y, likelihood, num_tasks_):
        ...
        self.task_covar_module = IndexKernel(num_tasks=num_tasks_, rank=1)
